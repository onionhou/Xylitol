...
function applicant(){
	...
	for(var i=0;i<afr.length;i++)
	{
		switch(i)
		{
			case 0: if(!name.test(afr[i].value))
					{
						fault(); 
						return false;
						}
			break;
			case 1: if(!tele.test(afr[i].value)){fault();return false;}
			break;
			case 2: if(!card.test(afr[i].value)){fault();return false;}else{$id("form").submit()}
			break;
			default:
			break;
		}
